...
import { BaZiResult, HuangliData, SolarTerm, ZodiacInfo, HourFortune } from '../calendar/lunar-adapter';
...
export class BaziCalculator {
  ...
  private day: number;
  private hour: number;
  private calendarType: 'solar' | 'lunar';
  ...
  // 天干对应的五行
  private static readonly STEM_ELEMENTS: Record<string, string> = {
    '甲': '木', '乙': '木',
    '丙': '火', '丁': '火',
    '戊': '土', '己': '土',
    '庚': '金', '辛': '金',
  ...
  // 地支对应的五行
  private static readonly BRANCH_ELEMENTS: Record<string, string> = {
    '子': '水', '丑': '土', '寅': '木', '卯': '木',
    '辰': '土', '巳': '火', '午': '火', '未': '土',
    '申': '金', '酉': '金', '戌': '土', '亥': '水'
  };
  ...
  public calculatePillars(): CalculationResult {
    try {
      // 创建适配器实例
      let adapter: LunarAdapter;
      if (this.calendarType === 'solar') {
        adapter = new LunarAdapter(this.year, this.month, this.day, this.hour, 0);
      } else {
        adapter = LunarAdapter.fromLunar(this.year, this.month, this.day, this.hour, this.isLeapMonth);
      }
      ...
      const dayMasterNature: '阳' | '阴' = yangStems.includes(bazi.dayMaster) ? '阳' : '阴';

      const result: CompleteBaZiData = {
        solarDate: {
          year: solarDate.year,
  ...
  // 计算五行分布
  private calculateFiveElementsDistribution(pillars: Array<{gan: string, zhi: string}>): Record<string, number> {
  ...
  // 计算贵人
  private calculateNobleman(dayStem: string): string[] {
    const yangStems = ['甲', '丙', '戊', '庚', '壬'];
    return yangStems.includes(dayStem) ? ['丑', '未'] : ['子', '申'];
  }

  // 计算桃花
  private calculatePeachBlossom(dayBranch: string): string {
  ...
  // 计算文昌
  private calculateIntelligenceStar(dayBranch: string): string {
  ...
  // 计算天马
  private calculateSkyHorse(monthBranch: string): string {
